...
<!-- 示例：每个功能都作为独立的库开始；库必须是自包含的、可独立测试的、有文档记录的；需要明确的目的——不包含仅用于组织内部的库 -->
...
<!-- 示例：文本 I/O 确保可调试性；要求结构化日志记录；或：MAJOR.MINOR.BUILD 格式；或：保持简单，YAGNI 原则 -->
...
## 治理
<!-- 示例：宪章优先于所有其他实践；修订需要文档记录、审批和迁移计划 -->

[GOVERNANCE_RULES]
<!-- 示例：所有 PRs/reviews 必须验证合规性；复杂性必须有正当理由；使用 [GUIDANCE_FILE] 进行运行时开发指导 -->
